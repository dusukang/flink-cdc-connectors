...
public class OceanBaseTableFactoryTest {
    ...
    private static final TableSchema SCHEMA_WITH_METADATA = TableSchema.builder()
            ...
            .add(TableColumn.metadata("tenant", DataTypes.STRING(),"tenant_name",true))
            .add(TableColumn.metadata("database", DataTypes.STRING(),"database_name",true))
            .add(TableColumn.metadata("table", DataTypes.STRING(),"table_name",true))
            .watermark(null)
            .primaryKey("pk",new String[]{"aaa"})
            .build();
